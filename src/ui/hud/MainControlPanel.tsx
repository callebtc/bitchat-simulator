...
const TinyGraph: React.FC<{ data: HistoryPoint[], color: string, label: string, maxValue?: number }> = ({ data, color, label, maxValue }) => {
    ...
    const points = useMemo(() => {
        ...
        return data.map(d => {
            const x = ((d.time - startTime) / HISTORY_DURATION_MS) * width;
    ...
    return (
        <div className="flex flex-col gap-0.5">
            ...
            <div className="border border-gray-800 bg-black/50 relative overflow-hidden" style={{ width, height }}>
...
export const MainControlPanel: React.FC = () => {
    ...
    return (
        <div className="absolute top-4 left-4 w-72 bg-black/90 text-white p-3 rounded backdrop-blur-md border border-gray-800 shadow-2xl font-mono select-none pointer-events-auto">
            {/* Header */}
            ...
            {/* Graphs */}
            <div className="flex gap-2 justify-between mb-4 border-t border-gray-800 pt-2">
                <TinyGraph 
